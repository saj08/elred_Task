...
const EditModalForm = (props) => {
    ...
    const [parentData, setParentData] = useState(orders[0].parentData);
    const [activeIndexPackaging, setActiveIndexPackaging] = useState(
        orders[0].activeIndexPackaging
    );
    const [activeIndexPackagingData, setActiveIndexPackagingData] = useState(
        orders[0].activeIndexPackagingData
    );
    const [oldActiveIndexPackagingData, setOldActiveIndexPackagingData] =
        useState(orders[0].activeIndexPackagingData);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            setSubmitting(true);
            try {
                const orderToUpdateIndex = orderData.findIndex(
                    (order) => order.id === oldActiveIndexPackagingData.variantId
                );
                if (orderToUpdateIndex !== -1) {
                    const updatedOrderData = [...orderData];
                    console.log(updatedOrderData[orderToUpdateIndex]);
                    updatedOrderData[orderToUpdateIndex] = {
                        ...updatedOrderData[orderToUpdateIndex],
                        quantity: values.quantity,
                        activeIndexPackagingData: activeIndexPackagingData,
                        id: activeIndexPackagingData.variantId,
                        activeIndexPackaging: activeIndexPackaging,
                    };
    ...
    const handleEditRowClick = (order) => {
        setParentData(order.parentData);
        setOldActiveIndexPackagingData(order.activeIndexPackagingData);
        setActiveIndexPackagingData(order.activeIndexPackagingData);
        setActiveIndexPackaging(order.activeIndexPackaging);
        formik.setFieldValue("quantity", order.quantity);
    };
    ...
    return (
        <>
            ...
            >
                ...
                >
                    <div>
                        <div className="fw-bold">{parentData.itemDescription}</div>
                        <div
                        ...
                        <div className="d-flex justify-content-center">
                            <img
                                src={
                                    parentData.productImages.length === 0
                                        ? "/product2.png"
                                        : parentData.productImages[0]
                                }
                                alt="gallery"
                            />
                        </div>
                            ...
                            <div className="col">
                                ...
                                >
                                    <div className="d-flex flex-wrap">
                                        {parentData.variants.map((variant, index) => (
                        ...
                        <tbody>
                            {orderData.map((order) => (
                                <tr
                                    ...
                                    onClick={() => handleEditRowClick(order)}
                                >
                                    <td>
                                        <img
                                            src={
